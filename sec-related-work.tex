...
specific applications. For example, \citet{Hamilton:1990:JoE} explores HMMs
in the context of ``regime-switching''. Essentially, these are HMMs with
observation distributions such as the multivariate Gaussian or an
auto-regressive model in order to properly capture real-valued vector data
in the domain of econometrics. Our model can be seen as such a
``regime-switching'' model where the output distributions are (observable)
Markov chains.
